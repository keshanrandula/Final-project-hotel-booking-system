...
export default function MyBookings() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        await fetchUserFeedbacks(token);
  ...
  // Fetch user feedbacks
  const fetchUserFeedbacks = async (token) => {
    try {
  ...
  const hasFeedback = (booking) => {
    // Since we don't have bookingId in feedback model, 
    // we'll check if there's a feedback with similar comment containing room name
    // This is a workaround - in production, add bookingId to feedback model
    return userFeedbacks.some(fb => 
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-gray-50 to-green-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {bookings.length === 0 ? (
          ...
          <div className="space-y-6">
            {bookings.map((booking) => {
              const alreadyHasFeedback = hasFeedback(booking);
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap justify-between items-start mb-4">
                      <div className="flex items-center space-x-2">
                        ...
                        {alreadyHasFeedback && (
                          <div className="bg-purple-100 text-purple-700 px-4 py-2 rounded-full text-sm font-semibold flex items-center">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                      ...
                      <div className="lg:col-span-1 flex flex-col justify-center space-y-3">
                        ...
                        {/* Feedback Button - Only show for paid bookings */}
                        {booking.isPaid && (
                          <button
                            onClick={() => openFeedbackModal(booking)}
                            className={`w-full px-4 py-3 rounded-xl font-semibold transition-all flex items-center justify-center shadow-md hover:shadow-lg ${
                              alreadyHasFeedback
                                ? 'bg-gradient-to-r from-purple-500 to-purple-600 text-white hover:from-purple-600 hover:to-purple-700'
                                : 'bg-gradient-to-r from-yellow-500 to-orange-500 text-white hover:from-yellow-600 hover:to-orange-600'
                            }`}
                          >
                            <Star className={`w-5 h-5 mr-2 ${alreadyHasFeedback ? 'fill-current' : ''}`} />
                            {alreadyHasFeedback ? 'Update Review' : 'Write a Review'}
                          </button>
                        )}
                      </div>
                    </div>

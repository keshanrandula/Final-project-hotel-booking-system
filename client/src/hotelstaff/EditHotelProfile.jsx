...
export default function EditHotelProfile() {
  ...
  useEffect(() => {
    const fetchHotelData = async () => {
      ...
      try {
        ...
        // Use the public route to fetch hotel data
        const res = await axios.get(`http://localhost:5000/api/hotels/${hotelId}`);
        ...
        console.error("Error fetching hotel:", err);
        setMessage({ 
          text: "Error loading hotel data. Please try again.", 
          type: "error" 
        });
      } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // IMPORTANT: Use the correct update route with /update suffix
      const res = await axios.put(
        `http://localhost:5000/api/hotels/${hotelId}/update`, // Added /update here
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (err.response?.data?.message) {
        ...
        errorMessage = "Hotel not found. Please check if you have permission to edit this hotel.";
      }
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              />
              <p className="text-sm text-gray-500 mt-1">
                Select new images to add to your hotel (existing images will be preserved)
              </p>
            </div>
